...
function classcard(num){
    return document.getElementsByClassName(`card${num}`);
}

function answer(drag,count) {
    console.log(drag.id);
    console.log(saveWords[1]);
...
function getCardsWords(words,kor,celebrity) {
    ...
    for (let word of words) {
        aCardHTML = `<div draggable = "true" id="${count++}"class="card${count}">${word}</div>`;
    ...
    saveWords = saveWords.join("","");
    cardsHTML = cardsHTML.join("","");
   
    
    
    // box 안에 위에서 생성한 카드들을 추가한다.
    document.querySelector(".ans").innerHTML = saveWords;
